...
#      end
    ...
    # get bucket or bucket list
    get '/' do
      only_authorized
      begin
        bucket = Bucket.find_root(bucket_name)
        get_bucket(bucket, params)
      rescue NoSuchBucket
        list_buckets
      end
    end
    ...
    # get slot (head)
    head %r{^/(.+)$} do
      slot_head(params[:captures].first, params)
      body ""
    end
    ...
    # create slot
    put %r{^/(.+)$} do
      create_slot(params[:captures].first, params)
    end

    # delete slot
    delete %r{^/(.+)$} do
      delete_slot(params[:captures].first)
    end
    ...
    def bucket_name
      request.env["SERVER_NAME"].match(/(^[^.]*)/).captures.first
    end
    ...
    def create_bucket
      begin
        ...
        raise AccessDenied if S3::Application.reserved_words.include?(bucket_name)
    ...
    def create_slot(slot_name, params)
      params.each do |k,v|
        case
          when k =~ /^x-amz-meta-(.*)$/
            @meta[$1] = v
      ...
      require 'pp'
      pp '*'*30, 'PARAMS', params, 'REQUEST', request, 'BODY', body
      contents = if params[:file].nil?
                   env['rack.input']
                 else
                   c = params[:file].instance_of?(File) ? params[:file] : StringIO.new(params[:file]) unless params[:file].nil?
                   env['CONTENT_LENGTH'] = c.length
                   c
                 end

      bucket = Bucket.find_root(bucket_name)
      begin
        slot = bucket.find_slot(slot_name)
      ...
      if params.has_key?('acl')
        ...
        fileinfo.etag='""' if slot_name[-8,8] == '$folder$'
        ...
        temp_path = contents[:path] rescue nil
        readlen = 0
        md5 = Digest::MD5.new

        Tempfile.open(File.basename(slot_name)) do |tmpf|
          temp_path ||= tmpf.path
          tmpf.binmode
          while part = contents.read(BUFSIZE)
            readlen += part.size
            md5 << part
            tmpf << part unless contents.is_a?(Tempfile)
          end
        end
      ...
      begin
        ...
        slot = Slot.create(:name => slot_name, :owner_id => owner_id, :meta => meta, :obj => fileinfo, :size => fileinfo.size)
